...
public static partial class HexConvert
{
    ...
    public static int GetBytes(ReadOnlySpan<char> source, Span<byte> buffer)
    {
        ...
        if (buffer.Length < requiredBufferSize) throw new Exception("HexConverterBufferCapacityException");
    ...
    {
        if (source.Length == 0) throw new Exception("HexConverterInvalidSourceLengthException");
    ...
    {
        if (source.Length == 0) throw new Exception("HexConverterInvalidSourceLengthException");
        if (buffer.Length < GetCharsCount(source)) throw new Exception("HexConverterBufferCapacityException");
...
public readonly struct RentedArraySegmentWrapper<T>(ArraySegment<T> arraySegment, ArrayPool<T> arrayPool) : IDisposable
{
    public ArraySegment<T> ArraySegment { get; } = arraySegment;
